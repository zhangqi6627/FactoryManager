...
public class PrintController implements Initializable {
	...
	private ObservableList<Order> getOrderList() {
		...
		ResultSet resultSet = DBUtil.getDBUtil().executeQuery("select * from orders where progress=3 or progress=4;");
		try {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		tc_receive.setCellFactory(new Callback<TableColumn<Order, String>, TableCell<Order, String>>() {
			...
			public TableCell<Order, String> call(final TableColumn<Order, String> param) {
				final TableCell<Order, String> cell = new TableCell<Order, String>() {
					final Button btn_receive = new Button("领取注塑半成品");
					...
					public void updateItem(String item, boolean empty) {
						...
						if (empty) {
							...
							btn_receive.setOnAction(event -> {
								...
								System.out.println("领取注塑半成品");
								/**
								 * 
								 * 
								 * TODO:弹出对话框提示大概需要多少注塑半成品，点击确认之后领取？？？
								 * 
								 * 
								 */
								orderList = getOrderList();
								...
								try {
									...
									System.out.println("原有注塑半成品:" + number + " 剩余注塑半成品:" + leftNumber);
									// 领取预计的塑料颗粒，更新products表中的物料信息
									int updateId = DBUtil.getDBUtil().executeUpdate("update products set number=" + leftNumber + " where product='注塑半成品';");
		...
		tc_start.setCellFactory(new Callback<TableColumn<Order, String>, TableCell<Order, String>>() {
			...
			public TableCell<Order, String> call(final TableColumn<Order, String> param) {
				final TableCell<Order, String> cell = new TableCell<Order, String>() {
					final Button btn_start = new Button("开始印刷");
					...
					public void updateItem(String item, boolean empty) {
						...
						if (empty) {
							...
							btn_start.setOnAction(event -> {
								Order order = getTableView().getItems().get(getIndex());
								// 更新进度为已派单，等待注塑车间接单
								boolean isSuc = DBUtil.getDBUtil().execute("update orders set progress=4 where _id=" + order.getId() + ";");
								if (isSuc) {
									System.out.println("开始印刷");
									orderList = getOrderList();
									tableView.setItems(orderList);
								} else {
		...
		tc_finish.setCellFactory(new Callback<TableColumn<Order, String>, TableCell<Order, String>>() {
			...
			public TableCell<Order, String> call(final TableColumn<Order, String> param) {
				final TableCell<Order, String> cell = new TableCell<Order, String>() {
					final Button btn_finish = new Button("完成印刷");
					...
					public void updateItem(String item, boolean empty) {
						...
						if (empty) {
							...
							btn_finish.setOnAction(event -> {
								Order order = getTableView().getItems().get(getIndex());
								boolean isSuc = DBUtil.getDBUtil().execute("update orders set progress=5 where _id=" + order.getId() + ";");
								if (isSuc) {
									System.out.println("完成印刷");
									orderList = getOrderList();
									tableView.setItems(orderList);
									...
									if (mUser != null) {
										...
										try {
											...
											// 在products中更新印刷半成品的数量
											DBUtil.getDBUtil().execute("update products set number=" + (number + productNumber)+" where product='印刷半成品';");
										} catch (SQLException e) {
											e.printStackTrace();
										}
									}
