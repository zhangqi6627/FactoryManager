...
public class ModelController implements Initializable {
	...
	private ObservableList<Order> getOrderList() {
		...
		ResultSet resultSet = DBUtil.getDBUtil().executeQuery("select * from orders where progress=1 or progress=2;");
		try {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		tc_receive.setCellFactory(new Callback<TableColumn<Order, String>, TableCell<Order, String>>() {
			...
			public TableCell<Order, String> call(final TableColumn<Order, String> param) {
				final TableCell<Order, String> cell = new TableCell<Order, String>() {
					final Button btn_receive = new Button("领取塑料");
					...
					public void updateItem(String item, boolean empty) {
						...
						if (empty) {
							...
							btn_receive.setOnAction(event -> {
								...
								System.out.println("领取塑料");
								orderList = getOrderList();
								tableView.setItems(orderList);

								int productNumber = order.getProductNumber();
								ResultSet resultSet = DBUtil.getDBUtil().executeQuery("select number from products where product='塑料颗粒';");
								try {
									int number = resultSet.getInt("number");
									int leftNumber = number - productNumber;
									System.out.println("原有塑料颗粒:" + number + " 剩余塑料颗粒:" + leftNumber);
									// 领取预计的塑料颗粒，更新products表中的物料信息
									int updateId = DBUtil.getDBUtil().executeUpdate("update products set number=" + leftNumber + " where product='塑料颗粒';");
									System.out.println("updateId:" + updateId);
									// 在records中添加一条记录
									if (mUser != null) {
										String username = mUser.getUsername();
										System.out.println("username:" + username);
										DBUtil.getDBUtil().execute("insert into records(material, number, username) values('塑料颗粒', " + (-productNumber) + ",'" + username + "');");
									}
								} catch (SQLException e) {
									e.printStackTrace();
								}
		...
		tc_start.setCellFactory(new Callback<TableColumn<Order, String>, TableCell<Order, String>>() {
			...
			public TableCell<Order, String> call(final TableColumn<Order, String> param) {
				final TableCell<Order, String> cell = new TableCell<Order, String>() {
					final Button btn_start = new Button("开始注塑");
					...
					public void updateItem(String item, boolean empty) {
						...
						if (empty) {
							...
							btn_start.setOnAction(event -> {
								Order order = getTableView().getItems().get(getIndex());
								// 更新进度为已派单，等待注塑车间接单
								boolean isSuc = DBUtil.getDBUtil().execute("update orders set progress=2 where _id=" + order.getId() + ";");
								if (isSuc) {
									System.out.println("开始注塑");
									orderList = getOrderList();
									tableView.setItems(orderList);
								} else {
		...
		tc_finish.setCellFactory(new Callback<TableColumn<Order, String>, TableCell<Order, String>>() {
			...
			public TableCell<Order, String> call(final TableColumn<Order, String> param) {
				final TableCell<Order, String> cell = new TableCell<Order, String>() {
					final Button btn_finish = new Button("完成注塑");
					...
					public void updateItem(String item, boolean empty) {
						...
						if (empty) {
							...
							btn_finish.setOnAction(event -> {
								 ...
								 * 
								 * 
								 * TODO:弹出一个对话框提示用户输入消耗了多少塑料颗粒？
								 * 
								 * 
								 */
								Order order = getTableView().getItems().get(getIndex());
								boolean isSuc = DBUtil.getDBUtil().execute("update orders set progress=3 where _id=" + order.getId() + ";");
								if (isSuc) {
									System.out.println("完成注塑");
									orderList = getOrderList();
									tableView.setItems(orderList);
									...
									if (mUser != null) {
										...
										String username = mUser.getUsername();
										System.out.println("username:" + username);
										// 在records中添加一条+注塑半成品的数据记录
										DBUtil.getDBUtil().execute("insert into records(material, number, username) values('注塑半成品', " + (productNumber) + ",'" + username + "');");
										try {
											ResultSet resultSet = DBUtil.getDBUtil().executeQuery("select number from products where product='注塑半成品';");
											int number = resultSet.getInt("number");
											System.out.println("number:" + number);
											// 在products中更新注塑半成品的数量
											DBUtil.getDBUtil().execute("update products set number=" + (number + productNumber)+" where product='注塑半成品';");
										} catch (SQLException e) {
											e.printStackTrace();
										}
									}
